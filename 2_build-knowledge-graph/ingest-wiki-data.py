...
def create_query_for_wiki_category_insertion(categories):
    """Create Cypher query to insert Wikipedia categories with error handling"""
    ...
    for i, category in enumerate(categories):
        ...
        # Escape any remaining quotes to prevent Cypher syntax errors
        safe_category = safe_category.replace('"', '\\"')
        
        query += f"""
            MERGE (category_{valid_categories}:Category {{code: "{safe_category}", title: "{safe_category}", description: "Generated by LLM for Wikipedia article"}})
        """
        valid_categories += 1
    
    # If we didn't add any categories, return an empty string
...
if category_query:  # Only execute if query is not empty
    try:
        ...
        for category in all_categories:
            try:
                single_query = create_query_for_wiki_category_insertion([category])
                if single_query:
    ...
    print("No valid categories to insert")
